    ...
    with get_db_session() as session:
        ...
        for raw_team in bootstrap_data.teams:
            try:
                ...
                logger.error(f"Failed to ingest team {raw_team.id if hasattr(raw_team, 'id') else 'unknown'}: {e}")
        ...
        for raw_player in bootstrap_data.elements:
            try:
                ...
                if not db_team_id:
                    raise ValueError(f"Team FPL ID {raw_player.team} not found in database")
                
                db_player = session.query(Player).filter_by(fpl_id=raw_player.id).first()
                if not db_player:
                ...
                db_player.chance_of_playing_next_round = raw_player.chance_of_playing_next_round
                db_player.chance_of_playing_this_round = raw_player.chance_of_playing_this_round
                db_player.news = raw_player.news
                session.flush()
                player_id_map[raw_player.id] = db_player.id
            except Exception as e:
                logger.error(f"Failed to ingest player {raw_player.id if hasattr(raw_player, 'id') else 'unknown'}: {e}")

        # 3. Ingest Fixtures
        for raw_fixture in fixtures_data:
            try:
                # Validation
                if not raw_fixture.id or not raw_fixture.team_h or not raw_fixture.team_a or raw_fixture.event is None:
                ...
                if not db_home_team_id or not db_away_team_id:
                    raise ValueError(f"Home/Away team FPL IDs ({raw_fixture.team_h}/{raw_fixture.team_a}) not found in database")
                
                db_fixture = session.query(Fixture).filter_by(fpl_id=raw_fixture.id).first()
                if not db_fixture:
                ...
                if raw_fixture.finished:
                    ...
                    for stat_category in raw_fixture.stats:
                        ...
                        for val in stat_category.h:
                            fixture_player_stats.setdefault(val.element, {})[db_field] = val.value
                        # Parse away stats
                    ...
                    for player_fpl_id, stats in fixture_player_stats.items():
                        ...
                        db_gw_stat = session.query(GameweekStat).filter_by(
                            player_id=db_player_id, 
                            gameweek=raw_fixture.event
                        ).first()
                        ...
                        db_gw_stat.total_points = stats.get("goals", 0) * 4 + stats.get("assists", 0) * 3 + stats.get("bonus", 0)  # Simplified calculation for fallback
                        session.flush()
                        
            except Exception as e:
                logger.error(f"Failed to ingest fixture {raw_fixture.id if hasattr(raw_fixture, 'id') else 'unknown'}: {e}")
