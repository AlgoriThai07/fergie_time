...
from sqlalchemy import (
    String, Boolean, Float, DateTime, ForeignKey, UniqueConstraint
)
...
class User(Base):
    ...
    fpl_entry_id: Mapped[Optional[int]] = mapped_column(unique=True, nullable=True, index=True)
    risk_profile: Mapped[str] = mapped_column(String(50), default="balanced")
    encrypted_session_token: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    # Relationships
    squads: Mapped[List["UserSquad"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    transfer_states: Mapped[List["UserTransferState"]] = relationship(back_populates="user", cascade="all, delete-orphan")
...
class Player(Base):
    ...
    position: Mapped[str] = mapped_column(String(10))  # e.g., "GKP", "DEF", "MID", "FWD"
    price: Mapped[int] = mapped_column()  # in tenths of a million, e.g. 55 = £5.5m
    status: Mapped[str] = mapped_column(String(10))  # e.g., "a", "i", "d"
    chance_of_playing_next_round: Mapped[Optional[int]] = mapped_column(nullable=True)
    chance_of_playing_this_round: Mapped[Optional[int]] = mapped_column(nullable=True)
    news: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    
    # Relationships
    team: Mapped["Team"] = relationship(back_populates="players")
    gameweek_stats: Mapped[List["GameweekStat"]] = relationship(back_populates="player", cascade="all, delete-orphan")
    squad_entries: Mapped[List["UserSquad"]] = relationship(back_populates="player", cascade="all, delete-orphan")
...
class UserSquad(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "gameweek", "player_id", name="uq_user_gameweek_player"),
    )
...
# TODO: recommendation_explanations (Sprint 3 — Grounded explanations for recommendations)
